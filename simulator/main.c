...
typedef struct {
    ...
    esp_bms_host_ble_client_t *host_ble;
...
static void restart_simulator(const host_app_t *app)
{
    ...
    lv_deinit();
#ifdef _WIN32
    _execvp(app->argv[0], (const char *const *)app->argv);
#else
...
static int host_set_environment(const char *name, const char *value)
{
#ifdef _WIN32
...
static bool apply_action_event(host_app_t *app, const esp_bms_lvgl_action_event_t *event)
{
    ...
    case ESP_BMS_LVGL_ACTION_TOGGLE_CONTROLLER_CONNECTION:
        ...
        if (app->host_ble &&
            !snapshot_flag_get(snapshot, ESP_BMS_DASHBOARD_FLAG_CONTROLLER_CONNECTION_ENABLED)) {
            (void)esp_bms_host_ble_client_disconnect(
                app->host_ble, ESP_BMS_HOST_BLE_SOURCE_CONTROLLER, snapshot);
        }
        refresh_speed_snapshot(app);
        return true;
    ...
    case ESP_BMS_LVGL_ACTION_START_BMS_BIND:
        ...
        if (esp_bms_lvgl_action_event_flag_get(
                event, ESP_BMS_LVGL_ACTION_EVENT_FLAG_BMS_MAC_VALID)) {
            return esp_bms_host_ble_client_connect(app->host_ble,
                                                   ESP_BMS_HOST_BLE_SOURCE_BMS,
                                                   snapshot->bms_type,
                                                   event->bms_mac,
                                                   snapshot);
        }
        return esp_bms_host_ble_client_start_scan(
            app->host_ble, ESP_BMS_HOST_BLE_SOURCE_BMS, snapshot);
    case ESP_BMS_LVGL_ACTION_START_CONTROLLER_BIND:
        if (!app->host_ble) {
            return false;
        }
        if (esp_bms_lvgl_action_event_flag_get(
                event, ESP_BMS_LVGL_ACTION_EVENT_FLAG_CONTROLLER_MAC_VALID)) {
            return esp_bms_host_ble_client_connect(app->host_ble,
                                                   ESP_BMS_HOST_BLE_SOURCE_CONTROLLER,
                                                   0U,
                                                   event->controller_mac,
                                                   snapshot);
        }
        return esp_bms_host_ble_client_start_scan(
            app->host_ble, ESP_BMS_HOST_BLE_SOURCE_CONTROLLER, snapshot);
    case ESP_BMS_LVGL_ACTION_CANCEL_BMS_CONNECTION:
        if (app->host_ble) {
            return esp_bms_host_ble_client_disconnect(
                app->host_ble, ESP_BMS_HOST_BLE_SOURCE_BMS, snapshot);
        }
...
int main(int argc, char **argv)
{
    ...
    if (headless && host_set_environment("SDL_VIDEODRIVER", "dummy") != 0) {
        perror("set SDL_VIDEODRIVER");
        return 1;
    }
    ...
    const char *host_ble_endpoint = getenv("ESP_BMS_SIMULATOR_BLE_ENDPOINT");
    if (host_ble_endpoint && host_ble_endpoint[0] != '\0') {
        app.host_ble = esp_bms_host_ble_client_create(host_ble_endpoint);
        if (app.host_ble) {
            esp_bms_host_ble_client_prepare_snapshot(app.host_ble, &app.snapshot);
            (void)esp_bms_lvgl_ui_update(&app.snapshot);
            fprintf(stderr, "simulator: host BLE bridge connected at %s\n", host_ble_endpoint);
        } else {
            fprintf(stderr, "simulator: failed to connect host BLE bridge at %s\n",
                    host_ble_endpoint);
        }
    }
    print_help(argv[0]);
    ...
    uint32_t last_host_ble_tick = SDL_GetTicks();
    ...
    while (app.running && frame < frame_limit) {
        ...
        if (app.host_ble) {
            ...
            if (esp_bms_host_ble_client_tick(app.host_ble, &app.snapshot,
                                             now - last_host_ble_tick)) {
                app.controller_speed_kmh_deci = app.snapshot.controller_speed_deci_units;
                refresh_speed_snapshot(&app);
                snapshot_changed = true;
            }
            last_host_ble_tick = now;
        }
    ...
    esp_bms_host_ble_client_destroy(app.host_ble);

...
- `status=planning` + required artifacts complete + required jsonl curated or inline mode → **1.4** (reuse explicit approval of the unchanged final plan from the conversation or existing task records; otherwise request final review approval before `task.py start`. Artifact existence alone is not approval.)

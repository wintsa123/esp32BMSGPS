...
4. LVGL 轮播与 V4 仪表
   ...
   - 使用单个自定义绘制对象实现 32 段色带、轮廓、刻度、卫星/状态点和缩小后的电池；标签实现速度、单位、电耗、中文温度缩写、默认挡位和本地时间，不显示 SOC 百分比文字。
   - 用一个布局函数切换横竖屏几何；移除来源变化触发的页面结构重建。
   - 数据更新严格执行无效显示、BMS 离线隐藏电池/电耗、控制器离线温度隐藏、超量程色带 clamp 和主数字保真。
...
6. 预览与静态验证
   ...
   - 渲染并查看横竖屏 0、88、中间、满量程、无效、GPS 搜星/定位、控制器在线/离线、BMS 离线、SOC 缺失状态；不满意则迭代。
   - 运行字体字形检查，确认 dashboard 温度所需“控”“电”“机”与设置中文字形均已存在。
...
- 三套 `settings_zh_10/13/16` 字体均包含“控”“电”“机”；速度仪表 host selftest、FarDriver selftest、Web 内联脚本语法与 `git diff --check` 通过。
- 真实 LVGL 9 预览已重渲染并逐张检查横屏 0/88/120/180/invalid/mph 与竖屏正常/GPS 搜星/控制器离线/SOC 缺失状态；横屏状态栏为单行，百分比文字消失，离线挡位为 `1`，未见裁切或重叠。
- ESP-IDF 5.5.4 构建通过；固件 `0x15b540`，最小 OTA 槽剩余 `0x84ac0`（28%）。GitNexus 最终检测为 MEDIUM，仅影响 7 个预期的仪表创建/绘制/布局符号和 5 条对应流程。
- 已通过 `rfc2217://192.168.2.10:4000?ign_set_control`、115200 刷写 MAC `20:e7:c8:5f:ab:a4`，所有写入 Hash 校验通过。
- 真机监视至 `uptime_s=300`：GPS UART 持续收到 RMC，UTC 连续推进，系统无 panic/WDT；但从 0/60/120/180/240/300 秒均为 `fix=0 pps=0`，启动 GGA 报告卫星数 `00`。因此 GPS 数据链路通过，室外首次定位/PPS 本轮未通过；未在无人操作条件下声称横竖屏触控旋转和控制器重连已验证。
- 用户将天线继续放置室外后再次刷写验证：启动 GSV 已报告约 5 颗可见卫星；`uptime_s=84` 收到首次 PPS，120 秒为 `fix=1 pps=1`，144 秒记录 `PPS stable: count=79`，180 秒仍为 `fix=1 pps=1`，期间无 panic/WDT。GPS 状态点应随 `GPS_FIX_VALID` 从橙色切换为绿色。
- 新增 BMS 离线横竖屏真实 LVGL 预览：`BMS_ONLINE=false` 时电池图形和电耗 label 完全隐藏，GPS、速度、默认挡位与时间几何不变；BMS 在线回归状态仍显示电池、电量段和电耗。

...
- 滚动容器在 `SCROLL_BEGIN` 记录稳定页和初始滚动位置，在 `SCROLL` 持续记录实际拖动位移，并在 `SCROLL_THROW_BEGIN` 冻结释放位移；不依赖子控件可能截获的 `PRESSED` / `RELEASED` 原始目标。冻结位移达到页宽 1/5 则按方向选择相邻页，否则回到当前稳定 `page`；该阈值依据历史触摸日志中有效横滑 P25=68px、抖动集中在 39px 以下确定。`SCROLL_END` 动画到显式目标后再刷新延迟快照，程序化 `move_to_page()` 不产生 throw-begin 事件，不受手势限幅。

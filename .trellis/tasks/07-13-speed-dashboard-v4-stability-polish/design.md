...
当前 `speed_dashboard_smooth_step()` 的 `3t^2-2t^3` 已等价于控制值
`0,0,1024,1024` 的三次贝塞尔标量曲线。用户提供文章中的定点精度扩展、浮点
版本和双核预计算不会解决当前主要问题，且会增加无必要的计算与同步复杂度。

最小方案是把逻辑/绘制段数从 32 提高到 48：横屏单段跨度从约 9.1 px 降到
约 6.1 px，同时活动段数最大值 48 仍可放入现有 render signature 的低 6 位，
无需改变签名字段布局。危险区起点保持在总段数的 `7/8`，次刻度步长保持总段数
的 `1/16`，主刻度步长保持总段数的 `1/4`。固件和 preview 共用相同比例，避免
新增控制点表、浮点贝塞尔或独立路径缓存。

48 段的宽线长度短于带宽，方端只重叠 1-2 px 会在下边缘留下黑色尖缝，圆端则
会把每个颜色段画成胶囊形。最终使用沿切线方向 4 px 的方端重叠。真正的
`lv_vector_path_cubic_to()` 需要当前关闭的 `LV_USE_VECTOR_GRAPHIC` 和 ThorVG；
软件矢量路径还要求至少 32 KB draw-thread stack，不适合无 PSRAM 的 WROOM-32E。

48 段会让色带主体和外轮廓 draw call 约增加 50%，因此必须复用现有 drag

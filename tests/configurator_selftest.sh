...
rg -qx '# CONFIG_ESP_BMS_LVGL_BRIDGE_FULL_REFRESH_DOUBLE_BUFFER is not set' "${repo_root}/config/sdkconfig/sdkconfig.defaults.esp32s3"
...
rg -Fq 'DASHBOARD_STATIC_CACHE_BYTES == 307200U' "${repo_root}/components/esp_bms_lvgl_ui/esp_bms_lvgl_ui_internal.h"
rg -Fq 'dashboard_static_cache_finalize(&s_ui.battery_static_cache' "${repo_root}/components/esp_bms_lvgl_ui/ui_bms_dashboard.c"
rg -Fq 'dashboard_static_cache_finalize(&s_ui.fireblade_static_cache' "${repo_root}/components/esp_bms_lvgl_ui/ui_dashboard_fireblade.c"
...
rg -qx 'MODULES=ble-media-hid,bms,controller,network,ota' "${work_dir}/s3-default-build/s3-default/firmware.env"
...
rg -Fq "module:cast) zh='手机投屏'; en='Phone casting'" "${repo_root}/start.sh"
rg -Fq "'module/cast' = @('手机投屏', 'Phone casting')" "${repo_root}/start.ps1"
